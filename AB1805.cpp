...
uint8_t AB1805::get_second_Hundredths(void) {
    return get_rtc_data(HUNDRETH_REGISTER, TENTH_MASK + HUNDRETH_MASK);
}
...
uint8_t AB1805::get_weekday(void) {
    return get_rtc_data(WEEKDAY_REGISTER, WEEKDAY_MASK);
}
...
{
  //rtc_check_valid();
  _second_Hundredths = get_second_Hundredths();
  _seconds = get_second();
  _minutes = get_minute();
  _hour = get_hour();
...
void AB1805::set_second_Hundredths(const uint8_t value) {
    _second_Hundredths = value % MAX_HUNDREDTHS;
    write_rtc_register(HUNDRETH_REGISTER, _second_Hundredths);
}
...
void AB1805::set_datetime(uint8_t year, uint8_t month, uint8_t date, 
  uint8_t weekday, uint8_t hour, uint8_t minutes, uint8_t seconds) {
    set_year(year);
    set_month(month);
    set_date(date);
    set_weekday(weekday);
...
bool AB1805::set_weekday_alarm(uint8_t value) {
    _alarm_weekday = value % MAX_WEEKDAY;
...
String AB1805::get_date_string(void){
  ...
  _second_Hundredths = get_second_Hundredths();
  date_string += ((_second_Hundredths & 0xF0) >> 4);
  date_string += (_second_Hundredths & 0x0F);
